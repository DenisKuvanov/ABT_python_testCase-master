...
def cls_method_decorator(param: int) -> Any:
    """
    Декоратор с аргументом, который принимает параметр param и внутри себя определяет функцию-обертку wrapper_func.
    Эта функция-обертка вызывает метод increment_var с указанным значением param, а затем вызывает исходный метод func.
    """
    def decorator(func: Callable):

        @wraps(func)
        def wrapper_func(self: "SomeClass", *args, **kwargs):
