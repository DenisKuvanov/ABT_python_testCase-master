...
async def main():
    ...
    for value in range(10):
        await server.handle_request(value)
    #TODO в данной задаче я не знаю как, не модифицируя существующий код, заставить
    # запросы работать последовательно. В обычном же случае я бы сделал как указал выше:
    # просто бы имитировал 10 запросов к серверу с помощью await.
    # Возможно это можно реализовать как-то с помощью очереди, но я пока не додумался
